...
1. **Parser-backed extraction**
   ...
   - Java Spring MVC controllers now emit deterministic `route` edges for `@GetMapping`, `@PostMapping`, and `@RequestMapping(... RequestMethod.X ...)` annotations, combining class-level prefixes with method-level paths so Java API slices expose interface maps to low-context workers.
   - C# ASP.NET controllers now emit deterministic `route` edges for `[Route]`, `[HttpGet]`, `[HttpPost]`, and related HTTP method attributes, combining class-level prefixes with method-level paths so .NET API slices expose interface maps to low-context workers.
